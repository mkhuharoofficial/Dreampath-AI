...
code = code.replace(/<motion\.div[\s\S]*?className="group p-6 bg-white border border-slate-200 rounded-3xl hover:border-indigo-600\/30 hover:shadow-lg hover:shadow-indigo-600\/5 transition-all cursor-pointer shadow-sm flex flex-col justify-between h-full"[\s\S]*?onClick=\{\(\) => onSelectDegree\(degree\)\}[\s\S]*?<\/motion\.div>/g, 
`<motion.button
  key={degree.id}
...
// One more place: The bookmarked/favorite ones at the top:
